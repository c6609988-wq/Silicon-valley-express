...
async function fetchDailyTopTweets() {
  ...
  // 4. 对 Top3 进行翻译 + 全量分析 + 存储
  const prompts = await getPrompts();
  let processed = 0;
  for (const tweet of top3) {
    try {
      const translated = tweet.content ? await deepseek.translateToZh(tweet.content) : '';
      const inputText = `博主：${tweet.author_name} ${tweet.author_handle || ''}\n日期：${tweet.published_at?.toISOString().split('T')[0]}\n原文：\n${tweet.content}\n\n翻译：\n${translated}`;
      const analysis = await deepseek.analyzeContent(prompts.SHORT_CONTENT_PROMPT, inputText);
