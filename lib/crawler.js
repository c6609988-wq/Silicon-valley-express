...
// ─── 每日 Top3 精选推文（主流程） ──────────────────────────────────────────
// followedIds: string[] | null — 传入时只采集该列表内的信源，null 表示采集全部
    ...
    try {
      ...
      const filtered = tweets
        ...
        .filter(t => {
          ...
          const stripped = text.replace(/https?:\/\/\S+/g, '').replace(/\s+/g, ' ').trim();
          // 去掉链接后实质文字需 > 20 字符，否则无内容可分析
          return text.length > 30 && stripped.length > 20
            && !(t.entities?.media?.length > 0 && stripped === '');
        })
        .slice(0, 5)
        .map(t => ({
          source_id: source.id,
